...
def classify_frame(frame, expected_class, visual_cues, debug=False, retries=2):
    ...
    prompt = (
        ...
        "or transition is UNMISTAKABLY visible — meaning the characteristic grip, "
        "body position, or limb configuration of that specific technique is clearly "
        "present in this frame. Demonstrations and drills count, but the technique "
        "must be identifiable on its own from the image alone.\n\n"
        f"CONTEXT (do NOT treat as the answer): this frame is from a '{hint}' video.\n"
        "IMPORTANT: The context is a weak hint only. Instructional videos often show "
        "setup positions, earlier steps, or explanations that look different from the "
        "actual technique. If the frame shows a DIFFERENT position than the context "
        "suggests, label it correctly. If no specific technique from the list below "
        "is clearly identifiable, return 'none'.\n\n"
        "When in doubt → 'none'. A wrong label is worse than a missing label.\n\n"
        "Respond with EXACTLY ONE label:\n\n"
        f"{class_list}\n  - none\n\n"
    )
    if visual_cues:
        prompt += f"Visual signature of '{hint}': {visual_cues[:250]}\n\n"
