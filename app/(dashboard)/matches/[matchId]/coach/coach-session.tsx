...
export default function CoachSession({
  ...
  const sendMessage = useCallback(async (userMessage: string) => {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        setMessages(prev => [...prev, { role: 'coach', text: 'Sorry, something went wrong. Try again.' }])
        return
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-3xl gap-0">
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b">
        <div>
          ...
          <h1 className="font-semibold text-sm mt-0.5">
            ...
            {match.eventName ? <span className="text-muted-foreground font-normal"> · {match.eventName}</span> : null}
          </h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-3 min-h-0">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`rounded-2xl px-3.5 py-2 text-sm max-w-[80%] ${msg.role === 'user' ? 'bg-foreground text-background rounded-tr-sm' : 'bg-muted rounded-tl-sm'}`}>
        ...
        {isListening && liveTranscript && (
          <div className="flex justify-end">
            <div className="rounded-2xl rounded-tr-sm px-3.5 py-2 text-sm max-w-[80%] bg-foreground/20 italic text-muted-foreground">
              {liveTranscript}
            </div>
          </div>
        )}
        ...
        {speechSupported ? (
          <div className="flex items-center gap-3">
            ...
            <p className="text-xs text-muted-foreground">
              {isListening ? 'Listening — release to send' : isLoading ? 'Coach is thinking…' : isSpeaking ? 'Coach is speaking…' : 'Hold to talk'}
            </p>
          </div>
        ) : null}
