...
export const scanUrl = inngest.createFunction(
  {
    ...
    // Cap simultaneous Gemini calls — prevents quota exhaustion and cascade OOM on server restart
    concurrency: { limit: 2 },
  },
  async ({ event, step }: {
    ...
    for (let i = 0; i < foundMatches.length; i++) {
      ...
      const matchId = await step.run(`create-match-${i}`, async () => {
        ...
        const [match] = await db.insert(matches).values({
          ...
          // The scan prompt instructs Gemini to report "seconds from video start" — these
