...
async function classifyFrame(
  ...
  const prompt = [
    ...
    'Look at this image and identify which BJJ technique, guard position, sweep, or transition is UNMISTAKABLY visible.',
    'The characteristic grip, body position, or limb configuration must be clearly identifiable in this frame alone.',
    '',
    `CONTEXT (do NOT treat as the answer): this frame is from a '${hint}' video.`,
    'IMPORTANT: The context is a weak hint only. Instructional videos often show setup positions or earlier steps that look different from the actual technique.',
    'If the frame shows a DIFFERENT position, label it correctly. When in doubt → "none". A wrong label is worse than a missing label.',
    '',
    'Respond with EXACTLY ONE label from this list, or "none" if no technique is clearly identifiable:',
    '',
    classList,
    '  - none',
    '',
